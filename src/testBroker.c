...
int main(int argc, char *argv[])
{
  ...
  void *frontEndToClient = zmq_socket(context,ZMQ_XREP);
  void *backEndToServer = zmq_socket(context,ZMQ_XREQ);
  
  zmq_bind(frontEndToClient, "tcp://*:6666");
  zmq_bind(backEndToServer,"tcp://*:5555");
  
  zmq_device(ZMQ_QUEUE, frontEndToClient, backEndToServer);
